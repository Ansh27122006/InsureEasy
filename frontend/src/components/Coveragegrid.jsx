...
const BanIcon = () => (
  <svg viewBox="0 0 22 22" fill="none" stroke="currentColor" strokeWidth={1.8}
    ...
    <line x1="4.22" y1="4.22" x2="17.78" y2="17.78" />
  </svg>
);

/* ── Section wrapper ──────────────────────────────────────────  */
function CoverageSection({ title, subtitle, badge, items, cardType, emptyMessage }) {
  ...
  return (
    <section className="flex flex-col gap-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div
            ...
            style={{ background: bg, color: text, boxShadow: `0 0 0 2px ${ring}` }}
          >
            <Icon />
          </div>
      ...
      {/* Divider */}
      <div className="h-px w-full rounded-full" style={{ background: ring }} />
...
/* ── CoverageGrid ─────────────────────────────────────────────  */
export function CoverageGrid({ coveredItems = [], excludedItems = [] }) {
  const totalItems = coveredItems.length + excludedItems.length;
            ...
            <div className="relative w-12 h-12 shrink-0">
              ...
              <svg viewBox="0 0 36 36" className="w-12 h-12 -rotate-90">
                ...
                <circle
                  ...
                  strokeDasharray={`${totalItems > 0 ? (coveredItems.length / totalItems * 88).toFixed(1) : 0} 88`}
                  strokeLinecap="round"
                  style={{ transition: "stroke-dasharray 0.8s ease" }}
                />
              </svg>
              <span className="absolute inset-0 flex items-center justify-center text-xs font-black text-gray-700">
                {totalItems > 0 ? Math.round(coveredItems.length / totalItems * 100) : 0}%
              </span>
            </div>
            <div>
              <p className="text-sm font-bold text-gray-800">Coverage Score</p>
              <p className="text-xs text-gray-400">
                {coveredItems.length} covered · {excludedItems.length} excluded
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full
              ...
              <svg viewBox="0 0 12 12" className="w-3 h-3" fill="currentColor">
                <path d="M2 6.5l3 3 5-5" stroke="currentColor" strokeWidth="1.5"
